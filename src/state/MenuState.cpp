...
namespace game {
    MenuState::MenuState(ContextPtr context, std::function<void()> on_quit) : m_context(std::move(context)),
                                                                              m_buttons(),
                                                                              m_text(),
                                                                              m_on_quit(std::move(on_quit)) {
        if (!m_context) {
            throw std::runtime_error("MenuState invalid context");
        }
        auto heading = std::make_unique<Text>(m_context->renderer, m_context->font_manager, "Flappy Bird",
                                              SDL_Rect{
                                                  (m_context->config.window_width / 2),
                                                  static_cast<int>(m_context->config.window_height * 0.2), 0, 0
        ...
        play_button_rect.x = (m_context->config.window_width / 2) - play_button_rect.w / 2;
        ...
        leaderboard_button_rect.x = (m_context->config.window_width / 2) - leaderboard_button_rect.w / 2;
        ...
        quit_button_rect.x = (m_context->config.window_width / 2) - quit_button_rect.w / 2;
    ...
    void MenuState::enter(Configuration const &config) {
    ...
    void MenuState::render(Registry const &) {
        m_text->render();
        for (auto &button: m_buttons) {
            button->render(m_context->renderer);
        }
    }
} // namespace game
