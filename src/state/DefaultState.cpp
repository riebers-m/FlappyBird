...
namespace game {
    ...
    void DefaultState::handle_events([[maybe_unused]] SDL_Event const &event) {}

    void DefaultState::update(entt::registry const &,
                              const std::chrono::milliseconds &dt) {
        m_context.input.update();
        m_context.systems_manager.get_system<systems::ScriptSystem>().update(
                dt);
    }

    void DefaultState::render(entt::registry const &) {
        m_context.systems_manager.get_system<systems::RenderSystem>().update(
                m_context.renderer, m_context.asset_store);
    }
} // namespace game
